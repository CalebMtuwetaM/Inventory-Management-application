...
def export_items(filename):
    with open(filename, 'w', newline='') as csvfile:
        fieldnames = ['item_name', 'item_id', 'item_description', 'total_number', 'cost_per_item', 'date_added', 'item_status']
    ...
    with open(filename, 'r') as csvfile:
        ...
        for row in reader:
            items.append({
                ...
                "total_number": int(row['total_number']),
                "cost_per_item": int(row['cost_per_item']),
                "date_added": datetime.strptime(row['date_added'], '%Y-%m-%d %H:%M:%S.%f'),
